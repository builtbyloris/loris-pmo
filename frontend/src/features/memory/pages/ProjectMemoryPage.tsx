import { Activity as ActivityIcon, ArrowLeft, CalendarClock, CheckCircle2, FileClock, Gavel, MessageSquare, NotebookPen, Plus, Search, Users } from "lucide-react";
...
export function ProjectMemoryPage() {
  ...
  const [commentTarget, setCommentTarget] = useState<{ type: "MEETING" | "DECISION"; id: string } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [nextProject, nextMembers, nextTasks, nextMilestones, nextLogs, nextMeetings, nextDecisions, nextActivity] = await Promise.all([
        projectsApi.get(projectId), peopleApi.listMembers(projectId), workPlanningApi.listTasks(projectId), workPlanningApi.listMilestones(projectId), memoryApi.listLog(projectId), memoryApi.listMeetings(projectId), memoryApi.listDecisions(projectId), can("audit.read") ? memoryApi.activity(projectId) : Promise.resolve({ items: [], total: 0 }),
      ]);
  ...
  const archived = Boolean(project.archived_at) || !can("meetings.manage");
  return <div className="memory-workspace page-stack">
    ...
    <div className="workspace-tabs" role="tablist" aria-label={t("memory.views.label")}>{(["log", "meetings", "decisions", ...(can("audit.read") ? ["activity" as const] : [])] as View[]).map((item) => <button key={item} role="tab" aria-selected={view === item} className={view === item ? "active" : ""} onClick={() => setView(item)}>{item === "log" ? <NotebookPen size={16} /> : item === "meetings" ? <Users size={16} /> : item === "decisions" ? <Gavel size={16} /> : <ActivityIcon size={16} />}{t(`memory.views.${item}`)}</button>)}</div>
