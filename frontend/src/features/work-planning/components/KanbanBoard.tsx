...
export function KanbanBoard({ tasks, milestones, members, readOnly, error, movingTaskId = "", onMove }: { tasks: Task[]; milestones: Milestone[]; members: ProjectMember[]; readOnly: boolean; error: string; movingTaskId?: string; onMove: (taskId: string, status: TaskStatus) => Promise<boolean> }) {
  ...
  return <div className="kanban-stack">{error && <div className="inline-error" role="alert"><AlertCircle size={15} />{error}</div>}<div className="kanban-board">{columns.map((status) => {
    ...
    return <section key={status} aria-label={t(`workPlanning.status.${status}`)} className={`kanban-column ${target === status ? "drop-target" : ""}`} onDragOver={(event) => { if (!readOnly && !movingTaskId) { event.preventDefault(); setTarget(status); } }} onDragLeave={() => setTarget(null)} onDrop={(event) => void drop(event, status)}><header><WorkBadge value={status} kind="status" /><span>{columnTasks.length}</span></header><div className="kanban-cards">{columnTasks.map((task) => { const assignees = task.assignee_ids.map((id) => members.find((member) => member.id === id)?.person.name).filter(Boolean); return <article key={task.id} aria-busy={movingTaskId === task.id} className={`kanban-card ${dragging === task.id ? "dragging" : ""} ${movingTaskId === task.id ? "saving" : ""}`} draggable={!readOnly && !movingTaskId} onDragStart={(event) => start(event, task.id)} onDragEnd={() => { setDragging(""); setTarget(null); }}><div className="kanban-card-top">{!readOnly && <GripVertical size={15} />}<WorkBadge value={task.priority} kind="priority" /></div><h3>{task.title}</h3>{assignees.length > 0 && <p className="task-assignees">{assignees.join(", ")}</p>}{!readOnly && <select className="kanban-status-select" aria-label={t("workPlanning.actions.changeStatus", { title: task.title })} value={task.status} disabled={Boolean(movingTaskId)} onChange={(event) => void onMove(task.id, event.target.value as TaskStatus)}>{columns.map((value) => <option key={value} value={value}>{t(`workPlanning.status.${value}`)}</option>)}</select>}{task.parent_task_id && <small>{t("workPlanning.subtask")}</small>}<div className="kanban-progress"><span><i style={{ width: `${task.completion_percentage}%` }} /></span><strong>{task.completion_percentage}%</strong></div>{task.milestone_id && <p>{milestones.find((item) => item.id === task.milestone_id)?.title}</p>}</article>; })}</div></section>;
  })}</div></div>;
}
